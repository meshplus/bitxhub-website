import {Box, BoxProps, createStyles, Grid, makeStyles, StyledComponentProps, Typography} from '@material-ui/core'
  ...
  return (
    <Box className={clsx(classes.card, className)} {...other}>
      ...
      <Box p={6} className={classes.cardContent}>
        <Typography mb={4} variant='h5' height={singleTitle ? '32px' : '64px'}>{title}</Typography>
        <Typography
          variant='subtitle1'>{desc}</Typography>
        <Typography mt={8} variant='body1'>{date}</Typography>
      </Box>
    </Box>
  )
}
